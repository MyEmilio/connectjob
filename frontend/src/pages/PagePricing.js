...
export default function PagePricing({ gs, update, navigate }) {
  ...
  const handleCheckout = async (planId) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: err.response?.data?.error || "Eroare la procesarea platii",
      });
    } finally {
  ...
  return (
    <div data-testid="page-pricing" style={{ animation: "fadeIn 0.3s ease" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <span
            ...
            style={{
              ...
              padding: "6px 16px",
              borderRadius: 999,
              background: PLAN_COLORS[current.plan]?.gradient || PLAN_COLORS.free.gradient,
              color: "#fff",
              fontSize: 12,
              fontWeight: 700,
          ...
          >
            ...
            {current.subscription?.current_period_end && (
              <span style={{ opacity: 0.8 }}>
                {" "}
                — expira {new Date(current.subscription.current_period_end).toLocaleDateString("ro")}
              </span>
            )}
          </span>
        </div>
            ...
            >
              ...
              <div style={{ textAlign: "center", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 2 }}>
                  ...
                  {plan.price > 0 && (
                    <span style={{ fontSize: 13, color: T.text3, fontWeight: 600 }}>RON/luna</span>
                  )}
                </div>
              </div>
              ...
              {isActive ? (
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    border: "none",
                    background: plan.id === "free" ? "#f5f5f4" : colors.gradient,
                    color: plan.id === "free" ? T.text3 : "#fff",
                    fontSize: 13,
                    fontWeight: 700,
                    fontFamily: "DM Sans,sans-serif",
                    cursor: plan.id === "free" ? "default" : "pointer",
                    boxShadow: plan.id !== "free" ? `0 4px 12px ${colors.accent}33` : "none",
      ...
      {gs?.user?.role === "worker" && (
        ...
        >
          ...
          <p style={{ fontSize: 12, color: T.text2, marginBottom: 14, lineHeight: 1.6 }}>
            Conecteaza-ti contul Stripe pentru a primi plati direct de la angajatori. Platforma
            retine un comision de 3% din fiecare tranzactie.
          </p>
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#166534" }}>
                  Cont Stripe conectat
                </div>
                <div style={{ fontSize: 11, color: "#166534", opacity: 0.8 }}>
                  Esti pregatit sa primesti plati.
                  {connectStatus.simulated && " (mod simulat)"}
                </div>
              </div>
            </div>
          ) : (
      ...
      >
        <p>
          Platforma retine un <strong style={{ color: T.text2 }}>comision de 3%</strong> din fiecare
          tranzactie procesata prin Stripe Connect.
        </p>
        <p>
          Planurile se pot anula oricand.{" "}
          {!plans[0]?.stripe_configured && (
            <span style={{ color: T.amber, fontWeight: 600 }}>
              Stripe nu este configurat — abonamentele functioneaza in mod demo.
            </span>
          )}
        </p>
      </div>
